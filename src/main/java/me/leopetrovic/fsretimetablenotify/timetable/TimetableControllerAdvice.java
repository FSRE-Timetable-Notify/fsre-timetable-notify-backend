...
public class TimetableControllerAdvice {
	@ExceptionHandler({
			TimetableFetchException.class,
			TimetableParseException.class
	})
	...
	public ResponseEntity<FsreError> handleTimetableException(Exception e) {
		final FsreError fsreException = new FsreError(HttpStatus.BAD_GATEWAY, "Bad Gateway",
	...
	@ApiResponses(value = {
			@ApiResponse(responseCode = "400", description = "Failed to parse ISO week", content = {
					@Content(mediaType = "application/json", schema = @Schema(implementation = FsreError.class))
			})
	})
	public ResponseEntity<FsreError> handleDateTimeParseException(DateTimeParseException e) {
		final FsreError fsreException = new FsreError(HttpStatus.BAD_REQUEST, "Bad Request",
