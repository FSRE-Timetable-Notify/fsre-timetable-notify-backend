...
public class TimetableDeserializer extends JsonDeserializer<Timetable> {
	...
	@Override
	public Timetable deserialize(JsonParser p, DeserializationContext ctxt) throws IOException, JacksonException {
		...
		for (JsonNode itemNode : itemsNode) {
			...
			Instant start = date.atTime(startTime).atZone(ZoneId.systemDefault()).toInstant();

			String endTimeStr = itemNode.get("endtime").asText();
			LocalTime endTime = LocalTime.parse(endTimeStr, TIME_FORMATTER);
			Instant end = date.atTime(endTime).atZone(ZoneId.systemDefault()).toInstant();
		...
		timetable.setMonday(eventsByDayOfWeek.getOrDefault(DayOfWeek.MONDAY, Collections.emptyList()));
		timetable.setTuesday(eventsByDayOfWeek.getOrDefault(DayOfWeek.TUESDAY, Collections.emptyList()));
		timetable.setWednesday(eventsByDayOfWeek.getOrDefault(DayOfWeek.WEDNESDAY, Collections.emptyList()));
		timetable.setThursday(eventsByDayOfWeek.getOrDefault(DayOfWeek.THURSDAY, Collections.emptyList()));
		timetable.setFriday(eventsByDayOfWeek.getOrDefault(DayOfWeek.FRIDAY, Collections.emptyList()));
		timetable.setSaturday(eventsByDayOfWeek.getOrDefault(DayOfWeek.SATURDAY, Collections.emptyList()));
		timetable.setSunday(eventsByDayOfWeek.getOrDefault(DayOfWeek.SUNDAY, Collections.emptyList()));
